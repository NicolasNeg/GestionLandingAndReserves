...
/**
 * Avatar / foto de perfil (mismas validaciones que producto).
 * @param {File} file
 * @param {string} uid
 */
export async function uploadAvatarImage(file, uid) {
  ...
  const safeName = `${Date.now()}_${Math.random().toString(36).slice(2, 10)}_${base}`;
  const storageRef = ref(storage, `avatars/${uid}/${safeName}`);
  await uploadBytes(storageRef, file, { contentType: type });
  return getDownloadURL(storageRef);
}
...
export async function uploadServiceImage(file, uid) {
  ...
  const base = file.name && file.name.includes('.') ? file.name.replace(/[^a-zA-Z0-9._-]/g, "_") : "servicio.jpg";
  const safeName = `${Date.now()}_${Math.random().toString(36).slice(2, 10)}_${base}`;
  const storageRef = ref(storage, `servicios/${uid}/${safeName}`);
  await uploadBytes(storageRef, file, { contentType: type });
  return getDownloadURL(storageRef);
}
